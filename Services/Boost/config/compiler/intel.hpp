...
// because they define it without a value, while Intel C++ defines it to 1.
// So we can check its value to see if the macro was defined natively or not.
// Under UNIX, the situation is exactly the same, but the macro _WCHAR_T is used instead.
