...
// Portions Copyright Shareaza Development Team, 2002-2004.
//
...
void CSHA::GetHash(Hashes::Sha1Hash& oHash) const
{
	std::transform( m_State.m_nState,
		m_State.m_nState + sizeof( m_State.m_nState ) / sizeof( m_State.m_nState[ 0 ] ),
		oHash.begin(), transformToBE< uint32 > );
	oHash.validate();
}

void CSHA::GetHash(Hashes::BtHash& oHash) const
{
	std::transform( m_State.m_nState,
		m_State.m_nState + sizeof( m_State.m_nState ) / sizeof( m_State.m_nState[ 0 ] ),
		oHash.begin(), transformToBE< uint32 > );
