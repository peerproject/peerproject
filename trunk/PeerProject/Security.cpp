...
{
	// No user agent- Assume bad. (Allowed connection by no searches performed)
	if ( sUserAgent.IsEmpty()  )										return TRUE;

	// Bad/unapproved/obsolete versions of Shareaza
	if ( _tcsistr( sUserAgent, _T("shareaza") ) )	
	{
		if ( _tcsistr( sUserAgent, _T("shareaza 0.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("shareaza 1.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("shareaza 2.0") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("shareaza 3.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("shareaza 4.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("shareaza 5.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("shareaza 6.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("shareaza 7.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("shareaza pro") ) )				return TRUE;

		return FALSE;
	}

	// Dianlei: Shareaza rip-off
	// Based on Alpha code, need verification for current status
	if ( _tcsistr( sUserAgent, _T("Dianlei") ) )
	{
		if ( _tcsistr( sUserAgent, _T("Dianlei 1.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("Dianlei 0.") ) )				return TRUE;
	...
	if ( _tcsistr( sUserAgent, _T("BearShare") ) )
	{
		if ( _tcsistr( sUserAgent, _T("BearShare 6.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("BearShare 7.") ) )				return TRUE;
		if ( _tcsistr( sUserAgent, _T("BearShare Lite") ) )			return TRUE;
		if ( _tcsistr( sUserAgent, _T("BearShare Pro") ) )			return TRUE;
		if ( _tcsistr( sUserAgent, _T("BearShare MP3") ) )			return TRUE;
		if ( _tcsistr( sUserAgent, _T("BearShare Music") ) )			return TRUE;
...
BOOL CSecurity::IsClientBanned(const CString& sUserAgent)
{
	...
	return Security.IsDenied( sUserAgent );
}
