...
BOOL CImageWindow::Refresh()
{
	...
	// Try to load the image, and convert it to RGB color if it is not already
...
{
	// The OnUpdate() method is invoked when PeerProject needs to update the state of a command in its
...
HRESULT STDMETHODCALLTYPE CImageWindow::OnCommand(UINT nCommandID)
{
	...
	// Note that IPluginWindowOwner::OnCommand() is different to ICommandPlugin::OnCommand() because
	// it is only called if the plugin window in question is active.
