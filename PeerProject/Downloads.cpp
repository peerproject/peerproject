...
CDownload* CDownloads::Add(CPeerProjectURL* pURL)
{
	if ( pURL->m_nAction != CPeerProjectURL::uriDownload &&
		 pURL->m_nAction != CPeerProjectURL::uriSource ) return NULL;
	...
	if ( pURL->m_sURL.GetLength() )
	{
		...
		{
			if ( pURL->m_nAction == CPeerProjectURL::uriSource )
			{
				delete pDownload;
				return NULL;
			}
...
void CDownloads::LoadFromCompoundFiles()
{
	...
	DeleteFile( Settings.Downloads.IncompletePath + _T("\\PeerProject Downloads.dat") );
	DeleteFile( Settings.Downloads.IncompletePath + _T("\\PeerProject Downloads.bak") );
	DeleteFile( Settings.Downloads.IncompletePath + _T("\\PeerProject.dat") );
	DeleteFile( Settings.Downloads.IncompletePath + _T("\\PeerProject1.dat") );
	DeleteFile( Settings.Downloads.IncompletePath + _T("\\PeerProject2.dat") );
}
...
void CDownloads::PurgeDeletes()
{
	...
	if ( ERROR_SUCCESS != RegOpenKeyEx( HKEY_CURRENT_USER,
		_T("Software\\PeerProject\\PeerProject\\Delete"), 0, KEY_ALL_ACCESS, &hKey ) ) return;
