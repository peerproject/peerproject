...
int CLibraryDictionary::ProcessPhrase(CLibraryFile* pFile, const CString& strPhrase, BOOL bAdd, BOOL bLowercase)
{
	...
	for ( ; *pszPtr ; nPos++, pszPtr++ )
	{
		...
		// for now, disable Numeric Detection in order not to split string like "PeerProject2" to "PeerProject 2"
		//if ( _istdigit( *pszPtr ) )
		...
		if ( !bCharacter || boundary[ 0 ] != boundary[ 1 ] && nPos )
		{
			...
			// but because PeerProject 2.2 and above are not really following GDF about
